...
namespace Codeplex.Data
{
    public class DynamicJson : DynamicObject
    {
        ...
        private static IEnumerable<XStreamingElement> CreateXObject(object obj)
        {
            ...
            {
                var list = new List<XStreamingElement>();
                foreach (var pair in obj as ExpandoObject)
                {
                    string name = pair.Key;
                    var val = pair.Value;

                    list.Add(new XStreamingElement(name, CreateTypeAttr(GetJsonType(val)), CreateJsonNode(val)));
                }
                return list.ToArray();
            }
        ...
        static public dynamic ConvertToExpandoObject(dynamic dy)
        {
            dynamic eo = new ExpandoObject();
            IDictionary<String, Object> dic = (IDictionary<String, Object>)eo;

            if (dy.IsObject)
            {
                foreach (string obj in dy.GetDynamicMemberNames())
                {
                    if (dy[obj] is DynamicJson)
                    {
                        dic[obj] = ConvertToExpandoObject(dy[obj]);
                    }
                    else
                    {
                        dic[obj] = dy[obj];
                    }
                    // CutSceneTool.Form1.writeConsole(obj);
                }
            }
            else if(dy.IsArray )
            {
                dynamic[] darry = (dynamic[])dy;
                List<dynamic> list = new List<dynamic>();
                foreach(dynamic d in darry)
                {
                    list.Add(ConvertToExpandoObject(d));
                }

                return (dynamic)list.ToArray();
            }
